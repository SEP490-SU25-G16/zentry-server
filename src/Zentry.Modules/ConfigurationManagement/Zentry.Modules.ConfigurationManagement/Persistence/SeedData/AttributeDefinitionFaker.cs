...
public class AttributeDefinitionFaker : Faker<AttributeDefinition>
{
    public AttributeDefinitionFaker(Guid id, string key, string displayName, string? description, DataType dataType,
        List<ScopeType> allowedScopeTypes, string? unit, string? defaultValue, bool isDeletable = true) // <-- Cập nhật tham số
    {
        CustomInstantiator(f =>
            AttributeDefinition.FromSeedingData(id, key, displayName, description, dataType, allowedScopeTypes, unit, defaultValue, isDeletable)); // <-- Cập nhật tham số
    }
    ...
    public AttributeDefinitionFaker()
    {
        ...
        RuleFor(ad => ad.AllowedScopeTypes, f => f.PickRandom(ScopeType.GetAll<ScopeType>(), f.Random.Int(1, 3)).ToList()); // Chọn 1-3 loại scope ngẫu nhiên
