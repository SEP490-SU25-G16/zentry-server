...
{
    private BluetoothScan() : base(Guid.Empty)
    {
    } // For MongoDB
    ...
    public BluetoothScan(Guid deviceId, string beaconId, int rssi, DateTime timestamp) : base(deviceId)
    {
        ...
        BeaconId = !string.IsNullOrWhiteSpace(beaconId)
            ? beaconId
            : throw new ArgumentException("BeaconId cannot be empty.", nameof(beaconId));
        Rssi = rssi;
        Timestamp = timestamp > DateTime.MinValue
            ? timestamp
            : throw new ArgumentException("Timestamp must be valid.", nameof(timestamp));
    }
