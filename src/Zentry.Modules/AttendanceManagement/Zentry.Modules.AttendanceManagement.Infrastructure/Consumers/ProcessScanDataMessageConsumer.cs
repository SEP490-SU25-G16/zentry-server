...
namespace Zentry.Modules.AttendanceManagement.Infrastructure.Consumers;

public class ProcessScanDataMessageConsumer : IConsumer<ProcessScanDataMessage>
{
    private readonly ILogger<ProcessScanDataMessageConsumer> _logger;
    private readonly IServiceScopeFactory _serviceScopeFactory; // To resolve scoped services like DbContexts

    public ProcessScanDataMessageConsumer(
    ...
    public async Task Consume(ConsumeContext<ProcessScanDataMessage> context)
    {
        ...
        _logger.LogInformation("MassTransit Consumer: Received scan data for RequestId: {RequestId}, Session: {SessionId}, Student: {StudentId}",
        ...
        try
        {
            ...
            _logger.LogInformation("MassTransit Consumer: Successfully processed scan data for RequestId: {RequestId}.", message.RequestId);
            // MassTransit automatically Acknowledges (ACKs) the message upon successful completion of Consume method.
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "MassTransit Consumer: Error processing scan data for RequestId {RequestId}. Message will be retried or moved to error queue.", message.RequestId);
