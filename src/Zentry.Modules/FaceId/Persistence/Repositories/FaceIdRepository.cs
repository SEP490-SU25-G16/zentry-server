...
public class FaceIdRepository : IFaceIdRepository
{
    ...
    public async Task<FaceEmbedding> CreateAsync(Guid userId, Vector embedding, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<FaceEmbedding> UpdateAsync(Guid userId, Vector embedding, CancellationToken cancellationToken = default)
    {
        ...
        var vectorString = "[" + string.Join(",", embeddingArray.Select(f => f.ToString("F6", System.Globalization.CultureInfo.InvariantCulture))) + "]";
        
        var sql = $"UPDATE \"FaceEmbeddings\" SET \"Embedding\" = '{vectorString}'::vector, \"UpdatedAt\" = NOW() WHERE \"UserId\" = '{userId}'";
    ...
    public async Task<(bool IsMatch, float Similarity)> VerifyAsync(Guid userId, Vector embedding, float threshold = 0.7f, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!exists)
            {
             ...
             var sql = $@"SELECT (1 - (""Embedding"" <=> '{vectorString}'::vector))::real FROM ""FaceEmbeddings"" WHERE ""UserId"" = '{userId}'";
