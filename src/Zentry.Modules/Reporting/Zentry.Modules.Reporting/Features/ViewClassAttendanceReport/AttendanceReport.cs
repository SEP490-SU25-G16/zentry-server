...
public class AttendanceReport
{
    ...
    private AttendanceReport() { } // For EF Core
    ...
    public AttendanceReport(Guid courseId, int totalStudents, int totalSessions, decimal averageAttendanceRate)
    {
        ...
        TotalStudents = totalStudents >= 0 ? totalStudents : throw new ArgumentException("TotalStudents cannot be negative.", nameof(totalStudents));
        TotalSessions = totalSessions >= 0 ? totalSessions : throw new ArgumentException("TotalSessions cannot be negative.", nameof(totalSessions));
        AverageAttendanceRate = averageAttendanceRate >= 0 && averageAttendanceRate <= 100
            ? averageAttendanceRate
            : throw new ArgumentException("AverageAttendanceRate must be between 0 and 100.", nameof(averageAttendanceRate));
