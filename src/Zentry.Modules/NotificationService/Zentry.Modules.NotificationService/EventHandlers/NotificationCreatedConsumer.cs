    ...
    INotificationSender notificationSender) : IConsumer<NotificationCreatedEvent>
{
    ...
    public async Task Consume(ConsumeContext<NotificationCreatedEvent> context)
    {
        ...
        logger.LogInformation("Received NotificationCreatedEvent for user {RecipientUserId}", notificationEvent.RecipientUserId);

        try
        {
            await notificationSender.SendNotificationAsync(notificationEvent, context.CancellationToken);
            logger.LogInformation("Successfully processed notification for user {RecipientUserId}", notificationEvent.RecipientUserId);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error processing notification for user {RecipientUserId}", notificationEvent.RecipientUserId);
