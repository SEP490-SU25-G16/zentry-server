...
public class FcmSender : IFcmSender
{
            ...
            if (FirebaseApp.DefaultInstance == null)
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(projectId) && !string.IsNullOrEmpty(privateKey) && !string.IsNullOrEmpty(clientEmail))
                    {
                        ...
                        var jsonConfig = $"{{\"type\":\"service_account\",\"project_id\":\"{projectId}\",\"private_key\":\"{privateKey}\",\"client_email\":\"{clientEmail}\"}}";
                    ...
                    else
                    {
                        ...
                        _logger.LogWarning("Or set environment variables: Firebase__ProjectId, Firebase__PrivateKey, Firebase__ClientEmail");
                    }
                }
            }
            else
        ...
        IReadOnlyDictionary<string, string>? data, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("Firebase not initialized. Skipping push notification for user {UserId}", recipientUserId);
            return;
        }
