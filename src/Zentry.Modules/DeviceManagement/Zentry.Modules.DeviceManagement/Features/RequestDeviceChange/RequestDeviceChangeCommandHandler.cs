...
using Zentry.SharedKernel.Constants.Attendance; // THÊM DÒNG NÀY ĐỂ TRUY CẬP RequestType
...
{
    public async Task<RequestDeviceChangeResponse> Handle(RequestDeviceChangeCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning("Device change request failed for user {UserId}: Max devices limit reached ({TotalDevices}/{MaxAllowed}).",
        ...
        {
            logger.LogWarning("Device change request failed for user {UserId}: Invalid device name '{DeviceName}'. Error: {Message}",
        ...
        {
            logger.LogWarning("Device change request failed for user {UserId}: Invalid MAC address '{MacAddress}'. Error: {Message}",
        ...
        if (existingDeviceByMac is not null)
        {
            if (existingDeviceByMac.UserId == command.UserId)
            {
                ...
                {
                    logger.LogWarning("Device change request failed for user {UserId}: MAC address {MacAddress} is already active for this user.",
                        command.UserId, command.MacAddress);
                    throw new BusinessRuleException("DEVICE_ALREADY_ACTIVE_FOR_USER", "Thiết bị này đã được đăng ký và đang hoạt động cho tài khoản của bạn.");
                }
                logger.LogWarning("Device change request for user {UserId}: MAC address {MacAddress} already exists but is not active. A new record will be created as per policy.",
                    command.UserId, command.MacAddress);
            }
            else
            {
                logger.LogError("Device change request for user {UserId}: MAC address {MacAddress} already exists for another user {ExistingUserId}. This indicates a potential data integrity issue.",
                    command.UserId, command.MacAddress, existingDeviceByMac.UserId);
                throw new BusinessRuleException("MAC_ADDRESS_TAKEN", "Địa chỉ MAC này đã được đăng ký bởi người dùng khác.");
            }
        }
        ...
        logger.LogInformation("New device {NewDeviceId} with MAC {MacAddress} created with status Pending for user {UserId}.",
        ...
        var requestUpdateDeviceMessage = new RequestUpdateDeviceMessage(
            ...
            RequestType: RequestType.UpdateDevice.ToString(),
            Reason: command.Reason
        );

        await publishEndpoint.Publish(requestUpdateDeviceMessage, cancellationToken);
        logger.LogInformation("MassTransit event RequestUpdateDeviceMessage published for user {UserId}, new device {NewDeviceId}.",
