...
public class RateLimitingService : IRateLimitingService
{
    ...
    public async Task<bool> CheckRateLimitAsync(string key, int maxAttempts, TimeSpan window)
    {
        try
        {
            ...
            {
                _logger.LogWarning("Rate limit exceeded for key {Key}. Current: {Count}, Max: {Max}", key, count, maxAttempts);
                return false;
            }
            ...
            _logger.LogDebug("Rate limit check passed for key {Key}. Current: {Count}, Max: {Max}", key, count + 1, maxAttempts);
            return true;
        }
        catch (Exception ex)
        {
