...
public class DeviceValidationMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        try
        {
            ...
            var deviceResponse = await mediator.Send(new GetDeviceByTokenIntegrationQuery(deviceToken), context.RequestAborted);
            if (deviceResponse.Device == null)
            {
                logger.LogWarning("Invalid device token {DeviceToken} for endpoint {Endpoint}", deviceToken, context.Request.Path);
                context.Response.StatusCode = 401;
